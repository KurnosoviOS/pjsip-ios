...
PJ_DEF(pj_status_t) pj_sock_socket(int af,
				   ...
				   pj_sock_t *sock)
{
    PJ_LOG(5,(THIS_FILE, "<--test-->pj_sock_socket invoked"));
    PJ_CHECK_STACK();
...
PJ_DEF(pj_status_t) pj_sock_bind( pj_sock_t sock,
				  ...
				  int len)
{
    PJ_LOG(5,(THIS_FILE, "<--test-->pj_sock_bind invoked"));
    PJ_CHECK_STACK();

    PJ_ASSERT_RETURN(addr && len >= (int)sizeof(struct sockaddr_in), PJ_EINVAL);
    PJ_LOG(5,(THIS_FILE, "<--test-->pj_sock_bind after assert"));

    CHECK_ADDR_LEN(addr, len);

    if (bind(sock, (struct sockaddr*)addr, len) != 0) {
       PJ_LOG(5,(THIS_FILE, "<--test-->pj_sock_bind pj_get_native_netos_error"));
  	   return PJ_RETURN_OS_ERROR(pj_get_native_netos_error());
    }
    else
	return PJ_SUCCESS;
